...
//Log message ID to differencitate between task
...
void vPipeline_Pose( void * pvParam )
{
    ...
    for(;;)
    {
        ...
        xQueueReceive( xQueue_to_pipeline,
                       raw_from_myo, 0); //never call task unless data is sent 
        //convert void ptr to 2D in array
        myo_raw_data = (int**)raw_from_myo
        ...
        //Send results back to top level task, aka maintask
        xQueueSend( xQueue_to_toplvl,
                    classifier_result, 0 );
